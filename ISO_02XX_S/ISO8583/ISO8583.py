...
"""
	...
	def getNetworkISO(self, bigEndian=True):
		...
		asciilen = len(asciiIso);
		netIso += str(asciilen).rjust(4, '0')
		netIso += asciiIso
	...
	def setNetworkISO(self,iso, bigEndian=True):
		...
		#if len(iso) != (size[0] + 4):
		#	raise InvalidIso8583('This is not a valid iso!!The ISO8583 ASCII(%s) is less than the size %s!' % (len(iso[4:]),size[0]))
			
		self.setIsoContent(iso[4:])	
